 ...
 * site menu items of the menu named 'main' or from multiple menu's as
 * configured by parameter 'breadcrumb-menus'. The content part of the
 * breadcrumb is generated from the resolved sitemap item belonging to the
 * current request.
 * <p>
 * The content items are based on the current resolved sitemap item and then
 * moving upwards, until the highest menu item is encountered, or, if the
 * 'breadcrumb-add-content-based' flag is up, until the site content base bean
 * is reached.
 */
public class BreadcrumbProvider {
	 ...
	 *
	 * @param component               component that creates this provider
	 * @param addTrailingDocumentOnly flag determining behaviour whether to add just
	 *                                one trailing document or all.
	 */
	@SuppressWarnings("unused")
	...
	public Breadcrumb getBreadcrumb(final HstRequest request) {
		...
		while (i < siteMenuNames.size() && deepestMenuItem == null) {
			...
			if (menu != null) {
				...
				log.debug("{} creating breadcrumb based on deepest menu item '{}' of menu '{}'",
						this.getClass().getName(), (deepestMenuItem == null) ? "null" : deepestMenuItem.getName(),
						menu.getName());
			}
			i++;
		}
		...
		if (linkNotFoundMode != null) {
			...
			log.info("{} post processed breadcrumb entries, using mode for not found links: {}",
					this.getClass().getName(), linkNotFoundMode);
		}
	...
	/**
	 * Returns the separator string that separates two breadcrumb items. It is
	 * configured by configuration parameter "breadcrumb-separator", defaulting to
	 * "»".
	 *
	 * @param request HST request
	...
	/**
	 * Generate the breadcrumb items which correspond to the expanded menu item
	 * tree.
	 *
	 * @param request  HST request
	 ...
	 * derived from the bean structure of the resolved sitemap item of the current
	 * request.
	 *
	 * @deprecated Renamed to getContentBreadcrumItems in version 1.5
	 */
	@Deprecated
	protected List<BreadcrumbItem> getTrailingBreadcrumbItems(final HstRequest request,
			final HstSiteMenuItem deepestExpandedMenuItem) {
	 ...
	 */
	protected List<BreadcrumbItem> getContentBreadcrumbItems(final HstRequest request,
			final HstSiteMenuItem deepestExpandedMenuItem) {
		...
		if (currentBean != null) {
			...
			if (deepestExpandedMenuItem != null) {
				...
				final HippoBean deepestExpandedMenuItemBean = getBeanForResolvedSiteMapItem(request,
						deepestExpandedmenuItemSmi);
				...
				if (addTrailingDocumentOnly) {
					...
					if (deepestExpandedMenuItemBean != null) {
						...
						if (!deepestExpandedMenuItemBean.isSelf(currentBean)
								&& deepestExpandedMenuItemBean.isAncestor(currentBean)) {
	 ...
	 * @param currentBean                 a bean described by URL that is in the
	 *                                    child tree of the ancestor bean
	 * @param deepestExpandedMenuItemBean bean corresponding to the deepest expanded
	 *                                    site menu item
	 * @param request                     HST request
	 */
	 ...
	 *
	 * @param items        list of breadcrumb items
	 * @param currentBean  a bean described by URL that is in the child tree of the
	 *                     ancestor bean
	 ...
	 * @param deepestExpandedmenuItemSmi deepest expanded site map item (its info
	 *                                   should be the first part of the
	 *                                   currentSmi's info for the implementation to
	 *                                   actually add items)
	 * @param request                    HST request
	 */
	protected void addURLBasedParentItems(final List<BreadcrumbItem> items, final HippoBean currentBean,
			final ResolvedSiteMapItem currentSmi, final ResolvedSiteMapItem deepestExpandedmenuItemSmi,
	...
	protected void addContentBasedItems(final List<BreadcrumbItem> items, HippoBean currentBean,
			final ResolvedSiteMapItem currentSmi, final HstRequest request) {
	...
	/**
	 * Post processes the breadcrumb items to check if the contained link points to
	 * the pagenotfound hst:page. If so, it sets the notFound flag in that link
	 *
	 * @param items   list of breadcrumb items
	 * @param request HST request
	 */
			...
			if (hstSiteMapItem != null) {
				...
				if (hstSiteMapItem.getComponentConfigurationIdMappings().size() != 0) {
					...
					if (resolvedSiteMapItem != null && HST_PAGES_PAGENOTFOUND_ID
							.equals(resolvedSiteMapItem.getHstComponentConfiguration().getName())) {
						item.getLink().setNotFound(true);
					}
				}
			}
	...
	protected BreadcrumbItem getBreadcrumbItem(final HstRequest request, final HippoBean bean,
		...
		if (navigationStateful) {
			return new BreadcrumbItem(context.getHstLinkCreator().create(bean.getNode(), context,
					null/* preferredItem */, true/* fallback */, navigationStateful), bean.getDisplayName());
		} else {
